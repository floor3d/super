...
int hkl_to_klid(HKL hkl, char out_klid[KL_NAMELENGTH]) {
  ...
  if ((device & 0xf000) == 0xf000) // `Device Handle` contains `Layout ID`
  {
    ...
    // Use input language only if keyboard layout language is not available.
    // This is crucial in cases when keyboard is installed more than once or
    // under different languages. For example when French keyboard is installed
    // under US input language we need to return French keyboard identifier.
    if (device == 0) {
      device = LOWORD(hkl);
    }
...
/*   // request query information permission on this thread */
